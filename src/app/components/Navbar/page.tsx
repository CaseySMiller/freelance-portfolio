    ...
    <Flowbite>
      <Navbar fluid rounded className='!bg-brand-purple'>
        ...
        <Dropdown
          arrowIcon={true}
          inline
          label={user.isLoggedIn ? <Avatar placeholderInitials={user.initials} rounded /> : <Avatar rounded />}
          // label={<Avatar placeholderInitials={user.initials} rounded />}
        >
          <Dropdown.Header>
            <span className="block text-sm">{user.isLoggedIn ? `${user.firstName} ${user.lastName}` : "Hello Guest"}</span>
            <span className="block truncate text-sm font-medium">{user.isLoggedIn ? user.email : ""}</span>
          </Dropdown.Header>
          <Dropdown.Item 
            href="/" 
